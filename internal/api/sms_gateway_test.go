...
/*
The three intervals, by hour and by whether the poll found anything. Pure: a
moment, the admin's number and the outcome in, the number the phone is told
out. Instants are given in UTC on purpose -- the box runs UTC, and the point
of the function is that it does not judge the hour by the box.
*/
func TestSMSGatewayPollSlowsWhenIdle(t *testing.T) {
	...
	cases := []struct {
		...
		{"mid-morning with work keeps the admin's rate", at(10, 0), 20, true, 20},
		{"mid-morning with nothing waits a minute", at(10, 0), 20, false, 60},
		{"dawn edge is daytime", at(6, 0), 20, true, 20},
		{"a minute before dawn is night", at(5, 59), 20, true, 900},
		{"last daytime minute", at(20, 59), 20, true, 20},
		{"21:00 sharp is night", at(21, 0), 20, true, 900},
		{"two in the morning is night", at(2, 0), 20, false, 900},
		{"night ignores the work it cannot have", at(2, 0), 20, true, 900},
		{"a slower admin setting survives the night", at(2, 0), 1200, false, 1200},
		{"the idle floor never shortens a slower setting", at(12, 0), 120, false, 120},
		{"daytime with the fastest allowed rate", at(12, 30), 5, true, 5},
	}
	for _, c := range cases {
		if got := smsGatewayPollFor(c.now, c.configured, c.found); got != c.want {
			t.Errorf("%s: smsGatewayPollFor(%s, %d, %v) = %d, want %d",
				c.name, c.now.In(ist).Format("15:04"), c.configured, c.found, got, c.want)
		}
	}
	// The daytime window is judged in India, not on the box: 00:30 UTC is
	// 06:00 IST and must read as day.
	if got := smsGatewayPollFor(time.Date(2026, time.September, 5, 0, 30, 0, 0, time.UTC), 20, true); got != 20 {
