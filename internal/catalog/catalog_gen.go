		...
		Sections: []Section{
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "super_admin.statutory_boards.digilocker_issuer_integration", Slug: "digilocker_issuer_integration", Name: "DigiLocker Issuer Integration", Scope: Scope("platform"), Tier: Tier("advanced"), Summary: "Not buildable here: needs NeGD issuer onboarding and DigiLocker's pull API credentials, which this deployment does not hold. Push transfer certificates, report cards and mark sheets as verifiable documents."},
			...
			},
			{
				...
				Features: []Feature{
					{Key: "super_admin.ai_automation.predictive_dropout_risk_engine", Slug: "predictive_dropout_risk_engine", Name: "Predictive Dropout Risk Engine", Scope: Scope("platform"), Tier: Tier("core"), Summary: "Rules, not a model: attendance under 75% in the last 30 days, a fee more than a month late, the last exam below pass. Counted per school; at risk on two of three. Names are read inside the school."},
					...
					{Key: "super_admin.ai_automation.ai_sentiment_analysis_on_feedback", Slug: "ai_sentiment_analysis_on_feedback", Name: "AI Sentiment Analysis on Feedback", Scope: Scope("platform"), Tier: Tier("core"), Summary: "Not buildable here: needs an NLP service, and the product stores no ratings to summarise honestly without one. Gauge campus sentiment from parent complaints and reviews."},
					{Key: "super_admin.ai_automation.automated_exam_question_translation", Slug: "automated_exam_question_translation", Name: "Automated Exam Question Translation", Scope: Scope("platform"), Tier: Tier("core"), Summary: "Not buildable here: needs a machine translation service this deployment does not have. Convert English question banks into regional languages."},
					{Key: "super_admin.ai_automation.smart_fee_cash_flow_predictor", Slug: "smart_fee_cash_flow_predictor", Name: "Smart Fee Cash Flow Predictor", Scope: Scope("platform"), Tier: Tier("core"), Summary: "What falls due in each of the next three months, times the share of the last twelve months' demand each school actually collected. Arithmetic on the row, not a model; the overdue backlog shown beside it."},
				},
			},
		},
	},
		...
		Sections: []Section{
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "institution_admin.academics.student_absentees", Slug: "student_absentees", Name: "Student absentees", Scope: Scope("institution"), Tier: Tier("core"), Summary: "A read-only day-wise monitor of every child marked away: their class and section, whether the parent has been called or is still pending, who made the call, and the reason the parent gave. Refreshes on its own as the office records calls, so the whole school's follow-up can be watched live without editing anything — the calling itself stays on Absentee followup."},
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "institution_admin.staff.my_pay", Slug: "my_pay", Name: "My pay", Scope: Scope("self"), Tier: Tier("core"), Summary: "Your own payslips month by month, what was taken off and how much of it was tax, the days you were marked present, and the leave you have left. Only ever your own."},
				},
			},
			{
				Slug:      "exams",
		...
		Sections: []Section{
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "faculty.attendance.student_absentees", Slug: "student_absentees", Name: "Student absentees", Scope: Scope("assigned_classes"), Tier: Tier("core"), Summary: "A read-only day-wise monitor of every child marked away: their class and section, whether the parent has been called or is still pending, who made the call, and the reason the parent gave. Refreshes on its own as calls are recorded, so a colleague's follow-up appears live without editing anything — the calling itself stays on Absentee followup."},
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "faculty.question_papers_online_tests.ai_examcell_paper_generator", Slug: "ai_examcell_paper_generator", Name: "AI Examcell Paper Generator", Scope: Scope("assigned_classes"), Tier: Tier("advanced"), Summary: "A paper drawn from the question bank by blueprint: so many of each difficulty, kind, chapter and mark value, at random. Nothing is written; a row the bank cannot fill says by how many."},
					{Key: "faculty.question_papers_online_tests.ved_ai_assessment_assistant", Slug: "ved_ai_assessment_assistant", Name: "Ved AI Assessment Assistant", Scope: Scope("assigned_classes"), Tier: Tier("advanced"), Summary: "Not buildable here: needs a language model to suggest rubric criteria, quiz questions and essay prompts, and this deployment has none."},
		...
		Sections: []Section{
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "finance.campus_money.store_catalogue", Slug: "store_catalogue", Name: "Store catalogue", Scope: Scope("institution"), Tier: Tier("core"), Summary: "The store's price list as families see it — every active product, its price and stock — so the counter and a parent are looking at the same shelf."},
				},
			},
			{
				Slug:      "accounts",
		...
		Sections: []Section{
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "admissions.applications.form_builder", Slug: "form_builder", Name: "Form Builder", Scope: Scope("institution"), Tier: Tier("core"), Summary: "Build the online application form the whole city fills in — add and order the fields, save drafts and publish a version — and open or close admissions for the season."},
		...
		Sections: []Section{
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "hr.attendance.staff_working_hours", Slug: "staff_working_hours", Name: "Staff Working Hours", Scope: Scope("institution"), Tier: Tier("core"), Summary: "The hours this school expects, named by the school and assigned to a department or a person: teaching, office and transport rarely start together. Each set says what counts as a full day, when somebody is late, and how -- or whether -- the school cuts pay for a day lost."},
					{Key: "hr.attendance.staff_hours_this_month", Slug: "staff_hours_this_month", Name: "Staff Hours This Month", Scope: Scope("institution"), Tier: Tier("core"), Summary: "One month of the fingerprint readers read against each person's own expected hours: days worked, half days, late arrivals, early departures, and the loss of pay that follows under the school's own rule -- with the rule named beside the figure."},
		...
		Sections: []Section{
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "parent.alerts_preferences.real_time_school_bus_live_video_feed_access", Slug: "real_time_school_bus_live_video_feed_access", Name: "Real-time School Bus Live Video Feed Access", Scope: Scope("children"), Tier: Tier("optional"), Summary: "Not built: needs bus-mounted cameras and a streaming vendor; live position is already in Live bus tracking. Permit authorized parents to view 15-second live video camera check-ins of child's bus during transit."},
			...
			},
			{
				...
				Features: []Feature{
					...
					{Key: "parent.messages.concerns", Slug: "concerns", Name: "Concerns", Scope: Scope("children"), Tier: Tier("core"), Summary: "Raise a concern with the school and follow it in one place — what you asked, who is handling it and their reply — instead of it living in a chat thread nobody can find later."},
