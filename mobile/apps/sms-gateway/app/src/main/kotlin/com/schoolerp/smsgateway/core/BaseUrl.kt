 ...
 *
 * HTTPS is not negotiable in a release build. A gateway carrying a bearer token
 * and the school's message traffic over plain HTTP on a mobile network is a
 * credential handed to whoever runs the Wi-Fi. `http://` is accepted only when
...
value class BaseUrl private constructor(val value: String) {
    ...
    /** Joins a contract path such as `/api/v1/sms-gateway/outbox`. */
