...
/* Shared by every /seller sub-file. */
...
/** Every /seller handler re-checks that the caller is the vendor, not a school admin. */
export function requirePlatformAdmin(c: Ctx): void {
  if (!c.id.platformAdmin) throw forbidden('platform administrators only')
}
