import { useState } from 'react'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import { CalendarOff, Clock } from 'lucide-react'
import { api, type List } from '@/lib/api'
import {
  PageHead, PageBody, Card, CardHeader, CellGrid, Stat, Table, Td, Badge,
  Button, ConfirmButton, Field, FormGrid, FormNotice, Input, Select, Textarea,
  Checkbox, Loading, ErrorState,
} from '@/components/ui'
import { formatDate } from '@/lib/utils'
import { useT } from '@/lib/i18n'
import { useChildren, childOptions } from './use-children'

/* Leave a family has asked for, and the form for asking.

   Both catalogued screens land here — the list and the application — because
   a parent opening "Requests" to apply is one tap from a list that answers
   "did I already ask?", and splitting them put that answer on another page.

   The form posts to /api/v1/workflow/leave, the endpoint the office's approval
   queue already reads. Writing to a parent-only endpoint would have produced a
   second pile of applications for the class teacher to miss. */

interface LeaveRow {
  id: string
  student_id: string
  student_name: string
  from_date: string
  to_date: string
  days: number
  is_half_day: boolean
  reason: string
  status: 'pending' | 'approved' | 'rejected' | 'cancelled'
  decision_note?: string
  decided_by?: string
  decided_at?: string
  applied_on: string
  cancellable: boolean
}

const TONE: Record<LeaveRow['status'], 'warning' | 'success' | 'danger' | 'neutral'> = {
  pending: 'warning',
  approved: 'success',
  rejected: 'danger',
  cancelled: 'neutral',
}

export default function LeaveRequests() {
  const t = useT()
  const qc = useQueryClient()
  const leave = useQuery({
    queryKey: ['portal-leave'],
    queryFn: () => api.get<List<LeaveRow>>('/api/v1/portal/leave'),
  })

  const cancel = useMutation({
    mutationFn: (id: string) => api.post(`/api/v1/portal/leave/${id}/cancel`, {}),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['portal-leave'] }),
  })

  if (leave.isLoading) return <Loading label={t('portal.leave_requests.loading')} />
  if (leave.error) return <ErrorState error={leave.error} />

  const rows = leave.data?.items ?? []
  const pending = rows.filter((r) => r.status === 'pending')

  return (
    <>
      <PageHead
        eyebrow={t('portal.leave_requests.eyebrow')}
        title={t('portal.leave_requests.title')}
        description={t('portal.leave_requests.description')}
      />
      <PageBody>
        <CellGrid cols={3}>
          <Stat label={t('portal.leave_requests.stat_waiting')} value={pending.length} icon={Clock} />
          <Stat label={t('portal.leave_requests.stat_approved')} value={rows.filter((r) => r.status === 'approved').length} />
          <Stat
            label={t('portal.leave_requests.stat_days')}
            value={rows.filter((r) => r.status === 'approved').reduce((n, r) => n + Number(r.days), 0)}
            icon={CalendarOff}
          />
        </CellGrid>

        <ApplyForLeave />

        <Card>
          <CardHeader
            title={t('portal.leave_requests.list_title')}
            description={t('portal.leave_requests.list_description')}
          />
          <Table
            head={[
              t('portal.leave_requests.col_child'),
              t('portal.leave_requests.col_days'),
              t('portal.leave_requests.col_reason'),
              t('portal.leave_requests.col_decision'),
              '',
            ]}
            empty={rows.length === 0}
            emptyLabel={t('portal.leave_requests.empty')}
          >
            {rows.map((r) => (
              <tr key={r.id}>
                <Td>
                  <div className="font-medium">{r.student_name}</div>
                  <div className="text-[12px] text-muted-foreground">
                    {t('portal.leave_requests.applied_on', { date: formatDate(r.applied_on) })}
                  </div>
                </Td>
                <Td>
                  {formatDate(r.from_date)}
                  {r.to_date !== r.from_date && ` – ${formatDate(r.to_date)}`}
                  <div className="text-[12px] text-muted-foreground">
                    {r.is_half_day
                      ? t('portal.leave_requests.half_day')
                      : Number(r.days) === 1
                        ? t('portal.leave_requests.days_one', { count: Number(r.days) })
                        : t('portal.leave_requests.days_many', { count: Number(r.days) })}
                  </div>
                </Td>
                <Td className="max-w-[20rem]">{r.reason}</Td>
                <Td>
                  <Badge tone={TONE[r.status]}>{r.status}</Badge>
                  {r.decision_note && (
                    <div className="text-[12px] text-muted-foreground">{r.decision_note}</div>
                  )}
                  {r.decided_by && (
                    <div className="text-[12px] text-muted-foreground">
                      {t('portal.leave_requests.decided_by', { name: r.decided_by })}
                    </div>
                  )}
                </Td>
                <Td>
                  {r.cancellable && (
                    <ConfirmButton
                      confirmLabel={t('portal.leave_requests.withdraw')}
                      question={t('portal.leave_requests.withdraw_question')}
                      onConfirm={() => cancel.mutate(r.id)}
                    >
                      {t('portal.leave_requests.withdraw')}
                    </ConfirmButton>
                  )}
                </Td>
              </tr>
            ))}
          </Table>
          <FormNotice error={cancel.error} />
        </Card>
      </PageBody>
    </>
  )
}

/** The application form. Powers parent.leave_absence.apply_student_leave. */
function ApplyForLeave() {
  const t = useT()
  const qc = useQueryClient()
  const { children, studentId, chosen, setChosen } = useChildren()
  const [from, setFrom] = useState('')
  const [to, setTo] = useState('')
  const [half, setHalf] = useState(false)
  const [reason, setReason] = useState('')

  const apply = useMutation({
    mutationFn: () =>
      api.post('/api/v1/workflow/leave', {
        student_id: studentId,
        from_date: from,
        // The shared endpoint requires both ends. A single day is the same date
        // twice rather than an absent field, which it would reject.
        to_date: half || !to ? from : to,
        is_half_day: half,
        reason,
      }),
    onSuccess: () => {
      setFrom('')
      setTo('')
      setHalf(false)
      setReason('')
      qc.invalidateQueries({ queryKey: ['portal-leave'] })
    },
  })

  const ready = studentId !== '' && from !== '' && reason.trim() !== ''

  return (
    <Card>
      <CardHeader
        title={t('portal.leave_requests.form_title')}
        description={t('portal.leave_requests.form_description')}
      />
      <div className="p-4">
        <FormGrid>
          {children.length > 1 && (
            <Field label={t('portal.leave_requests.field_child')} required>
              <Select
                value={chosen}
                onChange={setChosen}
                placeholder={t('portal.leave_requests.choose_child')}
                options={childOptions(children)}
              />
            </Field>
          )}
          <Field label={t('portal.leave_requests.field_first_day')} required>
            <Input type="date" value={from} onChange={setFrom} />
          </Field>
          {!half && (
            <Field
              label={t('portal.leave_requests.field_last_day')}
              hint={t('portal.leave_requests.field_last_day_hint')}
            >
              <Input type="date" value={to} onChange={setTo} />
            </Field>
          )}
          <Field label={t('portal.leave_requests.field_half_day')} wide>
            <Checkbox
              checked={half}
              onChange={setHalf}
              label={t('portal.leave_requests.half_day_label')}
              hint={t('portal.leave_requests.half_day_hint')}
            />
          </Field>
          <Field label={t('portal.leave_requests.field_reason')} wide required>
            <Textarea
              rows={2}
              value={reason}
              onChange={setReason}
              placeholder={t('portal.leave_requests.reason_placeholder')}
            />
          </Field>
        </FormGrid>
        <div className="mt-4">
          <Button disabled={!ready || apply.isPending} onClick={() => apply.mutate()}>
            {apply.isPending ? t('portal.leave_requests.sending') : t('portal.leave_requests.action_send')}
          </Button>
        </div>
        <FormNotice
          error={apply.error}
          ok={apply.isSuccess ? t('portal.leave_requests.sent_ok') : undefined}
        />
      </div>
    </Card>
  )
}
